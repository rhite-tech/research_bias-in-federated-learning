...
from data_loader import split_quantity_binary, load_data
from utils import get_state_metadata
...
def main(args):
    ...
    CLIENT_SIZES = np.round(np.linspace(1000, 1800000, NUM_CLIENTS)).astype(int)
    ...
    for SEED in SEEDS:
        ...
        clients = split_quantity_binary(metadata, int(NUM_CLIENTS/2))
        ...
        with open(path, 'a') as file:
            file.write(f'Results Experiments 4 (seed {SEED}) \n')
            file.write('-'*40)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_path', type=str, default='results/results_experiments_4.txt', help='Path to save the results')
