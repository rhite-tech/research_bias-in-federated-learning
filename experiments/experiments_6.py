...
from data_loader import load_data, split_label
...
def main(args):
    ...
    for SEED in SEEDS:
        ...
        clients, len_clients = split_label(data_folderpath, NUM_CLIENTS, SEED)
        trainloaders, valloaders, testloaders, complete_trainloader, complete_valloader, complete_testloader, missing = load_data(clients, SEED, batch_size=32, shuffle=True, missing=True)
        ...
        ###########################################
        print('\n Start running FL \n')
        fl_results = run_fl(model, trainloaders, valloaders, complete_trainloader, complete_valloader, DEVICE, len_clients, NUM_ROUNDS, FRACTION_FIT, FRACTION_EVALUATE, LEARNING_RATE)
        ...
        with open(path, 'a') as file:
            file.write(f'Results Experiments 6 (seed {SEED}) \n')
            file.write('-'*40)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_path', type=str, default='results/results_experiments_6.txt', help='Path to save the results')
