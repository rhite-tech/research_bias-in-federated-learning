...
from data_loader import arange_client_sizes, split_quantity_nonequal, load_data
...
def main(args):
    ...
    for SEED in SEEDS:
        ...
        client_sizes = arange_client_sizes(320, 100000, NUM_CLIENTS, 1736000)
        clients = split_quantity_nonequal(NUM_CLIENTS, client_sizes, data_folderpath, SEED)
        trainloaders, valloaders, testloaders, complete_trainloader, complete_valloader, complete_testloader, missing = load_data(clients, SEED, batch_size=32, missing=False)
        ...
        ###########################################
        print('\n Start running FL \n')
        fl_results = run_fl(model, trainloaders, testloaders, complete_trainloader, complete_testloader, DEVICE, NUM_CLIENTS, NUM_ROUNDS, FRACTION_FIT, FRACTION_EVALUATE, LEARNING_RATE)
        ...
        with open(path, 'a') as file:
            file.write(f'Results Experiments 2 (seed {SEED}) \n')
            file.write('-'*40)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_path', type=str, default='results/results_experiments_2.txt', help='Path to save the results')
